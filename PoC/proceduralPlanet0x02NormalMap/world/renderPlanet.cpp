...
RenderPlanet::RenderPlanet(PlanetFactory * planet) {
  ...
  glGenBuffers(1, &this->VBO[6]);
  glBindBuffer(GL_ARRAY_BUFFER, this->VBO[6]);
    glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec2) * this->planet->getVertexSize(), 0, GL_STATIC_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(glm::vec2) * this->planet->getVertexSize(), this->planet->texCoords);
  ...
  glBindAttribLocation(this->programID, 1, "inTexcoord");
...
void RenderPlanet::Render(int window_width, int window_height) {
  glUseProgram(this->programID);
    ...
    for(int i=0; i<6;i++) {
      ...
      glBindTexture(GL_TEXTURE_2D, this->planet->cubeMap->textureID[i]);
