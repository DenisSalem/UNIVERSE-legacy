...
PlanetFactory::PlanetFactory(int LOD, CubeMap * cubeMap) {
  ...
  // On map les coordonnés des textures dans un seul tableau parce qu'elle ne changerons pas.
  // Coordonnées sont directement déduite de celle des vertex.
  this->texCoords = new glm::vec2[this->vertexSize]; 
  ...
  for (int y=0; y<this->cubeScale; y++) {
    for (int x=0; x<this->cubeScale; x++) {
      ...
      h0 = (0.9 + 0.1 * (-this->cubeMap->min+this->cubeMap->displacementMaps[0][ this->cubeScale * y + x]) / this->cubeMap->max);
      h1 = (0.9 + 0.1 * (-this->cubeMap->min+this->cubeMap->displacementMaps[1][ this->cubeScale * (this->cubeScale -1 -y ) + x ]) / this->cubeMap->max);
      h2 = (0.9 + 0.1 * (-this->cubeMap->min+this->cubeMap->displacementMaps[2][ this->cubeScale * (this->cubeScale -1 -y ) + x ]) / this->cubeMap->max);
      h3 = (0.9 + 0.1 * (-this->cubeMap->min+this->cubeMap->displacementMaps[3][ this->cubeScale * y + x]) / this->cubeMap->max);
      h4 = (0.9 + 0.1 * (-this->cubeMap->min+this->cubeMap->displacementMaps[4][ this->cubeScale * (this->cubeScale-1-x) + y]) / this->cubeMap->max);
      h5 = (0.9 + 0.1 * (-this->cubeMap->min+this->cubeMap->displacementMaps[5][ this->cubeScale * (this->cubeScale-1-x) + this->cubeScale-1-y]) / this->cubeMap->max);

      this->texCoords[x + y * this->cubeScale].x = v1 + 0.5;
      this->texCoords[x + y * this->cubeScale].y = v2 + 0.5;
